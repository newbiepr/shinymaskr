...
(function () {
	...
	const ENVIRONMENT = "";
	    const DATA_URL = 'https://newbiepr.github.io/shinymaskr';
	    const DEV = false;
	...
	*/
	...
	const getStoryTitle = async () => {
	  if (!storyTitleLoaded) {
	    ...
	    if (!csv) {
	      csv = await fetchWithHash('/data/story-title.csv');
	...
	*/
	...
	const getProfile = async () => {
	  if (!profileLoaded) {
	    ...
	    if (!csv) {
	      csv = await fetchWithHash('/data/profile.csv');
	...
	const googleApi = async keyword => {
	  ...
	  const res = await request$1(`https://translate.google.com/translate_a/single?${query}`, {
	    data: data,
	    method: 'POST',
	    headers: {
	      'accept': '*/*',
